...
class RedisCallback:
    def __init__(self, host='127.0.0.1', port=6379, socket=None, password=None, key=None, numeric_type=float, **kwargs):
        """
...
class RedisZSetCallback(RedisCallback):
    async def write(self, feed: str, pair: str, timestamp: float, receipt_timestamp: float, data: dict):
        ...
        if self.redis is None:
            self.redis = await aioredis.create_redis_pool(self.conn_str, password=self.password)
